...
resource "aws_lambda_function" "lf-tf-1" {
  ...
  runtime          = "nodejs16.x"
  s3_bucket        = aws_s3_bucket.lf-tf-1-bucket.id
  s3_key           = aws_s3_object.hello-world-zip-s3.key
  source_code_hash = data.archive_file.zip_js_code.output_base64sha256
...
resource "aws_s3_object" "hello-world-zip-s3" {
  bucket = aws_s3_bucket.lf-tf-1-bucket.id
  key    = "hello-world.zip"
  source = data.archive_file.zip_js_code.output_path
  etag = filemd5(data.archive_file.zip_js_code.output_path)
}

resource "aws_cloudwatch_log_group" "hello" {
  name = "/aws/lambda/${aws_lambda_function.lf-tf-1.function_name}"
  retention_in_days = 5
}